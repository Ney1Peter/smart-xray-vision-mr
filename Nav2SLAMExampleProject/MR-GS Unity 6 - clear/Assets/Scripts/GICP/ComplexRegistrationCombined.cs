...
public class ComplexRegistrationManager : MonoBehaviour
{
...
*/    [DllImport("FGR_GICP.dll", CallingConvention = CallingConvention.Cdecl)]
    private static extern GICPResult RunFGRGICP(
        float[] refPoints, int refTotalFloats, float[] tgtPoints, int tgtTotalFloats,
        bool doDownsample, float voxelSize, int gicp_max_iter, float gicp_epsilon,
        bool useFGR, float voxelSizeFGR);
    ...
    private void InitiateRegistrationProcess(List<Vector3> targetCloud, string targetName )
    {
        ...
        GICPResult result = RunFGRGICP(
            ...
            doDownsample, voxelSize, gicp_max_iter, gicp_epsilon, useFGR, voxelSizeFGR);
