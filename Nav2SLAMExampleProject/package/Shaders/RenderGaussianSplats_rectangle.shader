...
Shader "Gaussian Splatting/Render Splats Rectangle"
{
    SubShader
    {
        ...
        Pass
        {
            ...
            /* ---------- Box (OBB) early rejection only ---------- */
            StructuredBuffer<float4> _BoxCenterHalf; // xyz = center, w = halfDepth
            StructuredBuffer<float4> _BoxAxisR;      // xyz = axisR (unit vector)
            StructuredBuffer<float4> _BoxAxisU;      // xyz = axisU (unit vector)
            StructuredBuffer<float4> _BoxAxisN;      // xyz = axisN (unit vector)
            ...
            /* ---------- Pixel-level rectangular mask (world-space params) ---------- */
            float3 _CutCenter;      // Center point (on wall)
            float3 _CutAxisR;       // Right axis (unit vector, in wall plane)
            float3 _CutAxisU;       // Up axis (unit vector, in wall plane)
            float3 _CutAxisN;       // Normal (unit vector, perpendicular to wall)
            float2 _CutRectHalf;    // Half width / half height (in meters)
            float  _CutMinAlpha;    // Minimum alpha inside hole (0 = fully transparent)

            /* ---------- Point cloud parameters (unchanged) ---------- */
            ...
            struct v2f
            {
                ...
                float3 worldPos : TEXCOORD1;   // Splat center in world space
            ...
            v2f vert(uint vtxID:SV_VertexID, uint instID:SV_InstanceID)
            {
                ...
                // Early rejection by OBB: drop splats inside box (makes cuboid "solid")
                if (InAnyBoxRegion(centerW)) {
                ...
                float4 centerC = _OptimizeForQuest ?
                    mul(UNITY_MATRIX_VP, float4(centerW, 1)) : view.pos;
                if (centerC.w <= 0) {
                    o.vertex = asfloat(0x7fc00000);
                    return o;
                }
            ...
            /* ---------- Fragment: pixel-level rectangle test ---------- */
            half4 frag(v2f i) : SV_Target
            {
                // 0) Gaussian alpha
                half alphaBase = exp(-dot(i.pos, i.pos));

                // 1) Ray from camera to pixel (in WS), intersect with wall plane
                ...
                if (abs(denom) > 1e-6)
                {
                    ...
                    if (t > 0.0)
                    {
                        float3 x = camWS + t * dirWS; // Intersection point (WS)
